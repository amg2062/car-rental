...
import { Bill, Bill2, Car, Image, Reservation, Reservation2, Review } from '../../model/car/Car';
...
export class CarService implements OnInit {
  ...
  constructor(private http: HttpClient,    @Inject(JWT_OPTIONS) private jwtOptions: any,private jwtHelper: JwtHelperService) { }
...
createReview(review: Review): Observable<any> {
  ...
  return this.http.post(
    `${environment.baseurl}/api/Cars/create-review`,
    review,
...
getReviewsByCarID(carID: number): Observable<Review[]> {
  ...
  return this.http.get<Review[]>(`${environment.baseurl}/api/Cars/reviews/${carID}`, { headers });
}

getImageByCarID(carID: number): Observable<Image> {
  const token = sessionStorage.getItem('token');
  return this.http.get<Image>(`${environment.baseurl}/api/Cars/image/${carID}`, { headers: { Authorization:  `Bearer ${token}`} });
}
insertImage(image: Image): Observable<any> {
  const token = sessionStorage.getItem('token');
  return this.http.post(
    `${environment.baseurl}/api/Cars/image`,
    image,
    { headers: { Authorization: `Bearer ${token}` } }
  );
}
